...
local L = LibStub("AceLocale-3.0"):GetLocale("Postal", true)
_G["Postal"] = Postal
...
Postal_DropDownMenu.UncheckHack = function(dropdownbutton)
	local check = _G[dropdownbutton:GetName().."Check"]
	local uncheck = _G[dropdownbutton:GetName().."UnCheck"]
...
-- Force profile submenus to grow downward from top of screen
local function repositionMenus()
	if UIDROPDOWNMENU_OPEN_MENU ~= Postal_DropDownMenu then return end
	local value = UIDROPDOWNMENU_MENU_VALUE
	if value ~= "SetProfile" and value ~= "CopyProfile" and value ~= "DeleteProfile" then return end
	for level = 3, UIDROPDOWNMENU_MAXLEVELS do
		local list = _G["DropDownList"..level]
		local parentList = _G["DropDownList"..(level - 1)]
		if list and list:IsShown() and parentList and parentList:IsShown() then
			local parentRight = parentList:GetRight() or 0
			local backdrop = list:GetBackdrop()
			list:ClearAllPoints()
			list:SetPoint("TOPLEFT", UIParent, "TOPLEFT", parentRight, 0)
			if backdrop then
...
repositionFrame:SetScript("OnUpdate", function(self, elapsed)
	if UIDROPDOWNMENU_OPEN_MENU ~= Postal_DropDownMenu then
		self:Hide()
		return
	end
...
pcall(hooksecurefunc, "ToggleDropDownMenu", function(level, value, dropDownFrame)
	if dropDownFrame == Postal_DropDownMenu then
		repositionMenus()
		repositionFrame:Show()
	end
end)

pcall(hooksecurefunc, "UIDropDownMenu_Refresh", function(dropDownFrame)
	if dropDownFrame == Postal_DropDownMenu then
		repositionMenus()
	end
end)
...
function Postal:OnInitialize()
	...
	for name, module in addon:IterateModules() do
		...
		if module.OnEnable then
			hooksecurefunc(module, "OnEnable", addon.OnModuleEnable_Common) -- Posthook
		end
	end
	...
	Postal_ModuleMenuButton:SetScript("OnClick", function(self, button, down)
		...
		ToggleDropDownMenu(1, nil, Postal_DropDownMenu, self:GetName(), 0, 0, nil, nil, "MENU")
	end)
...
function Postal:IsChatFrameActive(i)
	local _, _, _, shown, _, docked = FCF_GetChatWindowInfo(i);
...
function Postal:GetMoneyString(money)
	if GOLD_AMOUNT_TEXTURE then
		...
		return Postal:GetMoneyStringPlain(money)
	end
end
